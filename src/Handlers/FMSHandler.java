...
public abstract class FMSHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if(!hasRequiredAuthToken(exchange)) {
            FMSResponse response = new FMSResponse(new FMSError(FMSResponse.INVALID_AUTH_TOKEN_ERROR));
            sendResponse(HttpURLConnection.HTTP_UNAUTHORIZED,
                    GsonHelper.serialize(response), exchange);
            return;
        }
        ...
        try {
            ...
            response = new FMSResponse(new FMSError(FMSResponse.INVALID_REQUEST_DATA_ERROR+
                    ": " + e.getMessage()));
        } catch(Exception e) {
            response = new FMSResponse(new FMSError(FMSResponse.INTERNAL_SERVER_ERROR +
                    ": " + e.getMessage()));
        }
    ...
    private void sendResponse(FMSResponse response, HttpExchange exchange)
        ...
        sendResponse(getStatusCode(response), GsonHelper.serialize(response), exchange);
    }
